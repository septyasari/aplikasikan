                      ...
                      $hal = @$_GET['hal'];
                        if (empty ($hal)) {
                          ...
                          if ($_SERVER['REQUEST_METHOD'] == "POST"){
                            ...
                            if($pencarian != ''){
                              $sql = "SELECT*FROM tb_pemeriksa WHERE id_pemeriksa LIKE '%$pencarian%'";
                              $query = $sql;
                            $queryJml = $sql;                            
                            } else {
                              $query = "SELECT * FROM tb_pemeriksa LIMIT $posisi, $batas";
                              $query = "SELECT * FROM tb_pemeriksa";
                              $no = $posisi + 1;
                            }
                          } else {
                            $query = " SELECT * FROM tb_pemeriksa LIMIT $posisi, $batas";
                            $querJml = " SELECT * FROM tb_pemeriksa";
                            $no = $posisi + 1; 
                          }
                           $sql_pemeriksa = mysqli_query($con, $query) or die ( mysqli_error($con));
                          if(mysqli_num_rows($sql_pemeriksa) > 0){
                            while($data = mysqli_fetch_array($sql_pemeriksa)){
                              ?>
                              <tr>
                              <td><?=$data['id_pemeriksa']?></td>
                              <td><?=$data['bidang_yg_diperiksa']?></td>
                              <td><?=$data['nilai']?></td>
