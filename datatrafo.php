                      ...
                      $hal = @$_GET['hal'];
                        if (empty ($hal)) {
                          ...
                          if ($_SERVER['REQUEST_METHOD'] == "POST"){
                            ...
                            if($pencarian != ''){
                              $sql = "SELECT*FROM tb_trafo WHERE id_seri LIKE '%$pencarian%'";
                              $query = $sql;
                            $queryJml = $sql;                            
                            } else {
                              $query = "SELECT * FROM tb_trafo LIMIT $posisi, $batas";
                              $query = "SELECT * FROM tb_trafo";
                              $no = $posisi + 1;
                            }
                          } else {
                            $query = " SELECT * FROM tb_trafo LIMIT $posisi, $batas";
                            $querJml = " SELECT * FROM tb_trafo";
                            $no = $posisi + 1; 
                          }
                           $sql_trafo = mysqli_query($con, $query) or die ( mysqli_error($con));
                          if(mysqli_num_rows($sql_trafo) > 0){
                            while($data = mysqli_fetch_array($sql_trafo)){
                              ?>
                              <tr>
                              <td><?=$data['id_seri']?></td>
                              <td><?=$data['up3']?></td>
                              ...
                              <td><?=$data['beratminyak']?></td>
                              <td><?=$data['berattotal']?></td>
                              <td class="text-center">
                                <a href="edit.php?id=<?=$data['id_trafo']?>"><i class ="fas fa-edit"></i></a>
                                <a href="delete.php?id=<?=$data['id_trafo']?>"><i class ="fas fa-trash-alt"></i></a>
                            </td>
                            </tr>
                            <?php
